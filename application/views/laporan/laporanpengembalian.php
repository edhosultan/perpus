...
<head>
    <title>Cetak PDF Laporan Pengembalian</title>
    <style>
        h3 {
            font-size: 20px;
...
<body>
    ...
    if (!empty($_GET['tahun'])) {
        ...
        $tahun = $_GET['tahun'];
        $query = "SELECT * FROM tbl_pinjam WHERE year(tgl_kembali)='$tahun'";
    ...
    <table class="table table-bordered" style="width:100%;">
        ...
        <tbody>
            ...
            $jumlahtotal = 0;
            usort($kembali, function ($item1, $item2) {
                return $item2['jumlah'] <=> $item1['jumlah'];
            });

            foreach ($kembali as $isi) {
            ?>
                <tr>
                    <td><?php echo $no ?></td>
                    ...
                    <td><?php echo date("d-m-Y", strtotime($isi['tgl_balik'])) ?></td>
                    <td><?php echo date("d-m-Y", strtotime($isi['tgl_kembali'])) ?></td>
